...
**Honest read on the credit-risk number**: ROC-AUC 0.586 is weak — barely above the 0.5 no-skill baseline. This is not smoothed over: the synthetic applicant generator in `setup_data.py` mixes a fairly noisy latent default probability with only six features and a plain `LogisticRegression`, so a mediocre score here is an accurate reflection of a deliberately simple example model, not a bug. The point of this repo is the tool-calling architecture, not squeezing AUC out of a toy dataset.
